...
//
// 1) Compute the “last year Monday” origin & how many weeks (columns).
//
...
//
// 2) Resize the canvas to fit the grid exactly:
//
...
//
// 3) Generate a perfect maze with randomized DFS:
//
...
//
// 4) Convert your GitHub repos into floating dots at their creation‐week/day:
//
...
//
// 6) Initialize or restart the game:
//
function initGame() {
  frame = 0;
  gameOver = false;
  ...
  fetch('https://api.github.com/users/Dani-Nade/repos?per_page=100')
    .then(r => r.json())
    .then(data => scatterWithRepos(data))
    .catch(_ => {
      // fallback dummy grid
