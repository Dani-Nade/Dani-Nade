// docs/pacman-repo/script.js

window.addEventListener('load', () => {
  console.log('🚀 Pac-Man script starting…');
  ...
  function scatterWithRepos(repos) {
    ...
    console.log('Placed', dots.length, 'dots');
  }

  // 5) BFS to nearest dot
  function findPath() {
  ...
  function initGame() {
    ...
    pac.moving = false; pac.mouth = true;
    ...
    fetch('https://api.github.com/users/Dani-Nade/repos?per_page=100')
      ...
      .catch(err=>{
        console.error('Fetch failed, using dummy:', err);
  ...
  function drawScene() {
    ...
    ctx.arc(mx,my,r,ang+m,ang+Math.PI*2-m); ctx.fill();
  }
  ...
  function drawOverlay() {
    ...
    const txt='GAME OVER';
    const cols=['#f97583','#7ee787','#bb9af7','#58a6ff','#ff7b72','#3fb950','#ffab70','#b1bac4','#'];
    ctx.font='bold 48px monospace'; ctx.textBaseline='middle';
    let totalW=0;
    const ws = txt.split('').map(c=>{ const w=ctx.measureText(c).width; totalW+=w; return w; });
    let x=(canvas.width-totalW)/2, y=canvas.height/2;
    txt.split('').forEach((c,i)=> {
      ctx.fillStyle=cols[i]||'#c9d1d9';
      ctx.fillText(c, x+ws[i]/2, y);
      x+=ws[i];
    });
  }
